...
function CourseDetails() {
  ...
  // console.log(`course id: ${courseId}`)
    ...
    // Declare state to save the course details
  ...
  if (!response?.data?.courseDetails) {
    return (
      <div className="flex justify-center items-center h-[100vh]">
        <p className="text-3xl text-richblack-5">Course not found.</p>
      </div>
    )
  }
  ...
  const handleBuyCourse = () => {
    ...
    const isEnrolled = studentsEnrolled?.some(
      (student) => (student._id || student).toString() === user?._id
    )
  ...
  const handleAddToCart = () => {
    ...
    if (token) {
      if (cart?.some((item) => item._id === course_id)) {
      ...
      <div className="relative w-full bg-richblack-800">
        ...
        <div className="mx-auto box-content px-4 lg:w-[1260px] 2xl:relative ">
          <div className="mx-auto grid min-h-[450px] max-w-maxContentTab justify-items-center py-8 lg:mx-0 lg:justify-items-start lg:py-0 xl:max-w-[810px]">
            ...
            >
              <GiReturnArrow className="w-10 h-10 text-yellow-100 hover:text-yellow-50" />
            </button>

            {/* will appear only for small size */}
            <div className="relative block max-h-[30rem] lg:hidden">
              <Img
                src={thumbnail || "/default-course-thumbnail.png"}
            ...
            <div className="flex w-full flex-col gap-4 border-y border-y-richblack-500 py-4 lg:hidden">
              ...
              <button disabled={paymentLoading} className="yellowButton" onClick={handleBuyCourse}>Buy Now</button>
              <button disabled={paymentLoading} onClick={handleAddToCart} className="blackButton">Add to Cart</button>
            </div>
          </div>
